...
#
# Based on https://github.com/gvalkov/python-evdev/blob/main/evdev/evtest.py
#
...
def print_event(e):
    if e.type == ecodes.EV_SYN:
        ...
        up_down = None
        if e.value == 1:
            up_down = xkb.KeyDirection.XKB_KEY_DOWN
        else:
            up_down = xkb.KeyDirection.XKB_KEY_UP

        state.update_key(e.code + 8, up_down)
        if (e.type == ecodes.EV_KEY) and (e.value == 1):
            if e.code == ecodes.KEY_ENTER:
                sys.stdout.write('\n')
                sys.stdout.flush()
            else:
                sys.stdout.write(state.key_get_string(e.code + 8))
